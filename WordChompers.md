...
Each character (player and monster) will be of similar size, all able to fit fully inside of one grid cell. The animations will be idle, move, and eat. The player will have an extra idle animation, as it will be different depending on whether there is a word present in his current cell. The word in a cell will always be visible, except when there's a monster in the space. In that case, the word will appear behind them. For the player, the letter will render in front of him. The word cannot fill the leftmost 25% of the cell, as that space is reserved for part of the player.
